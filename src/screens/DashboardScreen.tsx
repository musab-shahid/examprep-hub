...
export function DashboardScreen() {
  ...
  return (
    <PageContainer>
      ...
      {/* Hero card */}
      <Card className="hero-glow p-5 sm:p-6 mb-4 bg-gradient-to-br from-sky-50 via-white to-slate-50 border-sky-100 animate-fade-in-up" style={{ animationDelay: '0.05s' }}>
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="min-w-0">
            <p className="text-lg sm:text-xl font-bold text-slate-900">{getGreeting()}!</p>
        ...
        <div className="grid grid-cols-4 gap-2 pt-3 border-t border-sky-100">
      ...
      <ActionCard className="mb-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <p className="text-xs font-semibold text-sky-600 uppercase tracking-wide">Today's Recommendation</p>
        </div>
        <p className="text-lg font-bold text-slate-900 mb-1">{recHeadline}</p>
        <p className="text-slate-500 text-sm mb-4">{recSubtext}</p>
        <button
          onClick={recNavigate}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-sky-500 text-white text-sm font-semibold hover:bg-sky-600 transition-colors shadow-sm shadow-sky-500/20"
        >
